window.onload = function () {
    ...
    // let sound1 = new Howl({
    //     src: [
    //         '../medicine/sound/failed.mp3',
    //         '../medicine/sound/failed.ogg'
    //     ]
    // }).play();
    // let sound2 = new Howl({
    //     src: [
    //         '../medicine/sound/refresh.mp3',
    //         '../medicine/sound/refresh.ogg'
    //     ]
    // }).play();
    ...
    let bee = {
        ...
        createDom: function () {
            ...
            this.frame.empty().css({
                    width: enemy.column * enemy.width + (enemy.column - 1) * enemy.space + "rem",
                    left: (innerWidth - (enemy.column * enemy.width + (enemy.column - 1) * enemy.space) * 100) / 2,
        ...
        readyToPlay:function () {
            ...
            // 然后缓缓飞出来
            _this.plane.css({ marginBottom:initBottom }).animate({ marginBottom:0 },_this.delay.plane,"easeOutBack",function () {
                // 获取敌机显示的间隔
        ...
        bumpCheck: function () {
            ...
            this.enemy.each(function () {
                ...
                if ( _this.isBump($("#js_spirit_1"), thisEnemy) || _this.isBump($("#js_spirit_2"), thisEnemy) ) {
                    _this.gamePause();
                    _this.gameLandmark( $("#js_pop_over") );
                }
                // 获取敌机的类型、血量及携带的额外分数
                let data = {
        ...
        isAllDestroyed: function () {
            ...
            if (_this.enemy.length === 0) {
                // 先清空定时器
                _this.gamePause();
                // 如果没有到最后一关，则关数+1，否则，显示通关
                if( _this.currentLevel <= levelData.length - 2 ){
                    _this.currentLevel++;
                    _this.gameLandmark( $("#js_pop_pass") );
                }else{
                    _this.gameLandmark( $("#js_pop_done") );
                }
                console.log("_this.currentLevel",_this.currentLevel);
        ...
        gameLoop: function () {
            let _this = this;
            // 获取延迟时间
            let delay = this.delay.plane + this.delay.enemy + 100;
            // 主循环
        ...
        // 暂停游戏并显示过关/失败/通关弹窗
        gameLandmark:function (obj) {
            // 游戏主界面淡出，并显示弹窗
            this.gameMain.fadeOut(200,function () {
                obj.addClass("scale-1");
            });
        },
        // 返回/再试一次/下一关
        gameOption:function () {
            let _this = this;
            // 点击按钮时
            // $(".js_pop_gaming").find(".btn").click(function () {
            //     // 先隐藏弹窗
            //     $(".scale-1").removeClass("scale-1");
            //     let that = $(this);
            //     // 判断点击的是取消还是确定，如果是取消，则返回主菜单，否则初始化游戏
            ...
            $("#js_pop_pass").find(".cancel").click(function () {
                $(".scale-1").removeClass("scale-1");
                before.ele.beforeMain.fadeIn();
            });
            $("#js_pop_pass").find(".confirm").click(function () {
                $(".scale-1").removeClass("scale-1");
                _this.gameStart();
            });
    ...
    let before = {
            ...
            ele.beforeMain.fadeOut(300,function () {
                ele.checkpoint.removeAttr("style").addClass("flex")
                    ...
                    setTimeout(function () {
                        ...
                        setTimeout(function () {that.removeClass("checkpoint_scale");},200);
                    },speed*index);
                    // 获取最高关卡数
        ...
        checkpointHide:function () {
            let ele = this.ele;
            // 选择关卡
            ele.allMenuPop.fadeOut(200,function () {
        ...
        settingInit:function () {
            ...
            bee.sound.music = storageSetting[2];
            bee.sound.effect = storageSetting[3];
        },
        // 游戏设置 - 弹出
        ...
        menuAnimation:function () {
            ...
            ele.plane.css({ top:0 });
            title.css({ top:0 });
            ele.menu.find("li").addClass("move_back");

        },
        // 菜单点击事件
        menuClick:function () {
            let _this = this;
            this.ele.menu.find("li").click(function () {
